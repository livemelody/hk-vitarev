...
class AW_Aheadmetrics_AheadmetricsController extends Mage_Core_Controller_Front_Action
{
    private $server = null;
    ...
    private function _init()
    {
        ...
        $this->auth_token = Mage::helper('awaheadmetrics/config')->getSecurityAuthKey();
    }
    ...
    public function setFullSyncAction()
    {
        $token = $this->getRequest()->getPost('key');
        if ($token) {
            $authkey = Mage::helper('awaheadmetrics/config')->getSecurityAuthKey();

            if (trim($authkey) && $token === $authkey) {
                Mage::helper('awaheadmetrics/config')->setFullsyncSalesReport(true);
                Mage::helper('awaheadmetrics/config')->setFullsyncCustomerReport(true);
            }
        }
