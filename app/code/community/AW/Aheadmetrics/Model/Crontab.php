...
class AW_Aheadmetrics_Model_Crontab
{
    ...
    private function _init()
    {
        ...
        $this->auth_token = Mage::helper('core')->decrypt(
    ...
    private function _auth()
    {
        ...
        if ($decodedResponse['success']) {
            ...
            throw new Exception(isset($decodedResponse['error_msg']) ? $decodedResponse['error_msg'] : $response);
        }
    }
    ...
    private function sendClientInfo()
    {
        $responseText = null;
        ...
        $this->_sendCurlRequest(
            '/backwardsync/version', http_build_query(
                array(
                    ...
                    'order_states' => json_encode($helper->getOrderStates()),
                )
            ), $responseText
        );
    }
    ...
    public function sync()
    {
        ...
        try{
            $model=Mage::getModel('awaheadmetrics/reports_salesreport');
            $model->processing();
            unset($model);
            $model=Mage::getModel('awaheadmetrics/reports_customerreport');
            $model->processing();
            unset($model);
        }catch (Exception $e){
            AW_Aheadmetrics_Helper_Data::logE($e);
