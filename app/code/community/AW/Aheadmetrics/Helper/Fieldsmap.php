...
class AW_Aheadmetrics_Helper_Fieldsmap
{
    ...
    protected function _loadMap($update = false)
    {
        ...
        if (!$this->_syncMap || $update) {
            if ($syncMap = @unserialize(Mage::getStoreConfig(self::CONFIG_NODE))) {
                $this->_syncMap = $syncMap;
                $this->_fieldMap = $this->_syncMap['fieldmap'];
                $this->_mapEntityTable = $this->_syncMap['relations'];
...
//update fiedmap from server
    ...
    public function getEntityMap($entity)
    {
        ...
        if (!isset($this->_mapEntityTable[$entity])) {
            return null;
        }
        if (array_key_exists($this->_mapEntityTable[$entity], $this->_fieldMap)) {
            return $this->_fieldMap[$this->_mapEntityTable[$entity]];
        }
    }
    ...
    {
        $this->_loadMap();
        $table = $this->_mapEntityTable[$entity];
        return (isset($this->_attributesMap[$table]) && in_array($column, $this->_attributesMap[$table]));
    }

    public function getEntityList()
    {
        $this->_loadMap();
        if (count($this->_mapEntityTable) == 0) {
            return array();
        }
        return array_keys($this->_mapEntityTable);
    }

    protected function _getDefaultMap()
    {
        $this->_mapEntityTable = array(
