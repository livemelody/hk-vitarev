...
class Licentia_Ecms_Adminhtml_Cms_PageController extends Mage_Adminhtml_Cms_PageController {
    ...
    public function massLayoutAction() {

        $pages = $this->getRequest()->getParam('page');
        $status = $this->getRequest()->getParam('root_template');
        ...
        if (!is_array($pages)) {
            ...
            try {
                foreach ($pages as $pageId) {
                    $model = Mage::getModel('cms/page')->load($pageId);
                    $model->setData('root_template', $status);
                    Mage::dispatchEvent('cms_page_prepare_save', array('page' => $model, 'request' => $this->getRequest()));
                    $model->save();
                }
                $this->_getSession()->addSuccess(
                        $this->__('Total of %d record(s) have their layout updated.', count($pages))
                );
            } catch (Exception $e) {
                $this->_getSession()->addError($e->getMessage() . ' ' . $this->__('No pages with layout updates'));
            }
        }
